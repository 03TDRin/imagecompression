...
def CompressHuffman(directory, filename, id):
    with rasterio.open(os.path.join(directory, filename)) as src:
        ...
        # Chuyển thành chunk nhỏ hơn để tránh MemoryError
        chunk_size = 1024 * 1024  # Xử lý 1MB một lần
        Compressed_data = []
        
        for i in range(0, len(Data_raster_byte), chunk_size):
            chunk = Data_raster_byte[i:i+chunk_size]
            Compressed_chunk = ''.join(Codebook[byte] for byte in chunk)
            Compressed_data.append(Compressed_chunk)
        
        # Kết hợp các phần đã nén
...
directory = "D:\\LE07_L2SP_125052_20181226_20200827_02_T1"
output_folder = "D:\\Nhóm 11 - PPNCKH"
def Menu():
